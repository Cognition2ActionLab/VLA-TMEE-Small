"""
    Adapted from <robomimic.utils.dataset>
...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.load_demo_info(filter_by_attribute=self.filter_by_attribute, train_ratio=train_ratio, train=train, val_demo_num=val_demo_num)
    ...
    def load_demo_info(self, filter_by_attribute=None, demos=None, train_ratio=None, train=True, val_demo_num=None):
        """
        Args:
        ...
        # split the demos into training and testing sets, if need
        if train_ratio != None:
            train_demo_num = round(train_ratio*self.n_demos)
            if train:
    ...
    def load_dataset_in_memory(self, demo_list, hdf5_file, obs_keys, dataset_keys, load_next_obs, train=None):
        """
        ...
        if train != None:
            if train:
                print("SequenceDataset: loading training dataset into memory...")
            else:
                print("SequenceDataset: loading validation dataset into memory...")
        else:
            ...
            for image_key in target_image_keys:
                ...
                for step_idx in range(obs_image.shape[0]):
                    ...
                    img_transfored = apply_image_perturbation(
                        ...
                        ptype=self.cfg.data.ptype,
                        variant=self.cfg.data.variant,
                        seed=0,
