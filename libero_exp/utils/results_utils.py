...
from .trans_image import apply_image_perturbation

@torch.no_grad()
def rollout(cfg, env_dict, policy, env_num, num_env_rollouts, horizon=None, return_wandb_video=False,
    ...
    for env_description, (env_idx, task_idx, env, init_states, task_emb) in env_dict.items():
        ...
        if video_writer != None:
            final_video_path = os.path.join(video_writer.video_path,
                                            str(task_idx) + '.' + env_description.split('/')[-1])
            if os.path.exists(final_video_path):
                files = os.listdir(final_video_path)
                if files != []:
                    scuccess_files = [file for file in files if file.endswith("True.mp4")]
                    all_succ += [len(scuccess_files) / num_env_rollouts]
                    all_horizon += [0]
                    vid += [0]
                    ...
                    continue
        for env_num_i in range(env_num):
            print(f"\nEnv: {env_idx}, Task_id: {task_idx}, Rollout: {env_num_i + 1}/{env_num} running...")
            env.reset()
            env.seed(cfg.train.seed)
            policy.reset()
            if video_writer != None:
                video_writer.reset()
            obs = env.set_init_state(init_states[env_num_i][np.newaxis, :])
            if cfg.policy.policy_head.network == "DPHead":
                action_dim = cfg.policy.policy_head.network_kwargs.in_channels
            else:
                action_dim = cfg.policy.policy_head.network_kwargs.output_size

            dummy_actions = np.zeros((num_env_rollouts, action_dim))
            for _ in range(5):
                env.step(dummy_actions)

            dones = [False] * num_env_rollouts
            num_success = 0
            episode_frames = []
            ...
            for step_i in tqdm(range(horizon)):
                if cfg.data.noise:
                    ...
                    img_agentview = cv2.cvtColor(img_agentview, cv2.COLOR_BGR2RGB)
                    img_eye_in_hand = cv2.cvtColor(img_eye_in_hand, cv2.COLOR_BGR2RGB)
                    past_img_agentview_list = [img_agentview.copy()]
                    past_img_eye_in_hand_list = [img_eye_in_hand.copy()]

                    img_agentview = apply_image_perturbation(
                        image=img_agentview,
                        category=cfg.data.category,
                        ptype=cfg.data.ptype,
                        variant=cfg.data.variant,
                        seed=cfg.train.seed,
                        past_image_list=past_img_agentview_list,
                    )
                    img_eye_in_hand = apply_image_perturbation(
                        image=img_eye_in_hand,
                        category=cfg.data.category,
                        ptype=cfg.data.ptype,
                        variant=cfg.data.variant,
                        seed=cfg.train.seed,
                ...
                if cfg.policy.policy_head.network == "DPHead":
                    a = policy.get_action(cfg, data).reshape(1, action_dim)
                else:
                    a = policy.get_action(cfg, data)
                obs, r, done, info = env.step(a)

                video_img = []
                for k in range(num_env_rollouts):
            ...
            if video_writer != None:
                video_writer.get_last_info(env_num_i, dones, env_description, task_idx)
                video_writer.save()

            for k in range(num_env_rollouts):
                num_success += int(dones[k])
            all_succ += [num_success / num_env_rollouts]
    ...
    for idx, (env_idx, task_idx) in enumerate(zip(all_env_indices, all_task_indices)):
        ...
        if return_wandb_video:
            results[f"rollout/vis_env{env_idx}_task{task_idx}"] = wandb.Video(env_vid[idx], fps=30, format="mp4",
                                                                              caption=all_env_descriptions[idx])
        else:
...
def merge_results(results: List[dict], compute_avg=True):
    ...
    if compute_avg:
        merged_results["rollout/horizon_env_avg"] = np.mean(
            np.array([v for k, v in merged_results.items() if "rollout/horizon_env" in k]).flatten())
        merged_results["rollout/success_env_avg"] = np.mean(
            np.array([v for k, v in merged_results.items() if "rollout/success_env" in k]).flatten())
